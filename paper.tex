...
\begin{abstract}
\noindent Research on the career network treats the effect of the relational structure on outcomes as invariant with time.
Among 495 elite Korean artists over seven decades (1929--2002), the association between network stability and career plateau risk is absent early but strengthens monotonically from year~10 to year~20, while network size remains protective throughout.
The pooled coefficient averages over these stages, describing a statistical artifact that corresponds to no actual career phase, regardless of causal direction.
Bidirectional feedback between network consolidation and productivity decline intensifies over career time, constraining all admissible causal models.
Quasi-experimental evidence from institution closures supports \textit{evaluative redundancy---diminishing} returns from repeated endorsements by the same gatekeepers---as a forward-acting pathway.
The association escalates across the achievement distribution.
A pilot replication in the film industry supports the phase reversal: a data-driven change-point model identifies overlapping thresholds in both datasets, and a structural break test confirms the pattern.
Replication code and data: \url{https://github.com/joonhyungbae/TITV}.
\end{abstract}
...
However, no studies have traced a reversal in the \textit{direction} of the network effect across the entire career trajectory.
...
In the present data, the pooled stability coefficient of 1.18 averages over a null effect in the first decade (\HR{} = 1.04) and a substantial effect thereafter (\HR{} = 1.39 in year 20), describing a statistical artifact that corresponds to no actually observed career stage.
...
The evaluation redundancy mechanism requires three structural conditions to generate the predicted phase reversal.
...
Full details of the construction pipeline are provided in the Appendix~\ref{app:data_construction}; the complete pipeline and enriched dataset are publicly available (see Data and Code Availability).
...
The lagged specifications ($S_{t-2}$) address assumption (ii) by blocking the contemporaneous reverse path; The bound assumption of E-values \citep{vanderweele2017sensitivity} (i); measured covariates ($A_t$, $t$, $N_t$) close additional back-door paths.
...
\label{eq:rcs_hazard}
\end{equation}
where $f_j(t)$ are the non-linear RCS basis functions of the career year.
...
A profile-likelihood change-point model complements the RCS specification: for each candidate $\tau \in \{5, 6, \ldots, 20\}$, phase-specific stability coefficients are estimated and AIC selects the optimal $\tau^*$ with a cluster-bootstrap of 95\% CI.
This data-driven approach eliminates the need for the researcher to specify the cut-off \textit{point a priori} (Section~\ref{sec:pilot_replication}).
...
Solo exhibitions and awards confer strong, long-lived but decaying reductions in plateau hazard (solo: \HR{} = 0.48 at 1 year, deteriorating to 0.67 at 10 years, half-life $\approx$ 20.5 years; awards: \HR{} = 0.63 at 1 year, half-life $\approx$ 17.5 years; both \pval{} $< 0.001$).
Even strong endorsements do not permanently protect careers from the consequences of network consolidation.
...
The most direct demonstration of phase reversal comes from a tertile career-phase model that divides the career into three stages (years 0--7, 8--14, 15+) and estimates phase-specific stability HRs, avoiding the multicollinearity problem of the continuous interaction entirely.
...
Conditional hazard ratios -- the substantively meaningful quantities -- are significant in each career year from 10 to (Figure~\ref{fig:stability_hr}), supporting Hypothesis~2a.
In absolute terms, moving from $-1$ SD to $+1$ SD in network stability increases the probability of the five-year plateau by 5.4 percentage points in career year 10 (from 13.5\% to 18.9\%) and by 7.6 percentage points in year 20 (from 9.0\% to 16.6\%) -- a moderate absolute effect whose practical significance lies in its monotonic escalation over career time.
...
Three additional analyzes confirm the robustness of the post-decade finding: a logistic regression in the post-decade subsample (OR = 1.41, \pval{} = 0.012), the sensitivity of the cut point from year 7 to 13 (stability HR consistently above 1.0, significant at cut points 8--11), and a leave-one-out analysis (all 37 specifications yield HR $> 1.0$, all \pval{} $< 0.05$).
...
The analyzes in this section address a further question: to what degree does the association reflect forward-acting evaluative redundancy, reverse-causal feedback, or both?
The evidence is organized in four parts: the temporal structure of bidirectional feedback, quasi-experimental evidence from institution closures, sensitivity analyzes, and an interpretive summary.
...
Critically, the main effect of the full-sample is slightly \textit{stronger} at lag-1 (\HR{} = 1.25) than contemporaneously (\HR{} = 1.18), while the post-decade estimates in Table~\ref{tab:causal_ordering} show a comparable pattern. The lag-2 specification, the preferred identification strategy, yields a conditional HR at year 10 of 1.23 (\pval{} = 0.002) and at year 15 of 1.31 (\pval{} = 0.004).
...
At 2- and 3-year horizons, both channels attenuate and become jointly non-significant (forward 2-year: joint $p = 0.306$; reverse 2-year: joint $p = 0.176$; forward 3-year: joint $p = 0.260$; reverse 3-year: joint $p = 0.279$), indicating that the Granger asymmetry is a short-horizon phenomenon.

Crucially, a stage-specific decomposition shows that this asymmetry is concentrated in the pre-decade career window.
In the first decade, the reverse path is marginally significant ($\beta = 0.034$, \pval{} = 0.061), while the forward path is significant and \textit{negative} ($\beta = -0.065$, \pval{} = 0.003), consistent with stability in the early-career that facilitates (rather than hinders) productivity growth.
...
A dose-response specification using the continuous pre-closure exposure share yields a significant gradient: each standard-deviation increase in institutional reliance at the time of closure is associated with lower post-closure plateau odds (OR $= 0.77$, \pval{} $= 0.039$), and this result survives the addition of a pre-closure productivity trend control (OR $= 0.77$, \pval{} $= 0.037$).
The result is robust to varying the closure cutoff between 1993 and 1999 (Fisher OR $= 0.53$, \pval{} $= 0.027$ in the most inclusive definition).
An event study specification that estimates the relative-years coefficients from $t-5$ to $t+5$ around the closure shows that the post-treatment coefficients are uniformly negative (significant at $t+4$: $\beta = -0.18$, \pval{} $= 0.006$), although the parallelism before the trend is imperfect (Wall \pval{} $= 0.019$), motivating the propensity score approach as the primary quasi-experimental estimator (Figure~\ref{fig:closure_es}).
Taken together, logistic, propensity-matched, dose-response, and productivity-conditioned closure analyzes provide convergent quasi-experimental evidence for a forward-acting component: artists who were forced to diversify more, and whose diversification was not driven by their own productivity trajectories, show lower subsequent plateau risk.
...
The E-values \citep{vanderweele2017sensitivity} quantify the minimum confounding strength required to explain each finding (Table~\ref{tab:evalue}).
...
The size of the network shows the greatest robustness (E-value = 2.22).
The Burt constraint does not show an independent association once size and stability are controlled (\HR{} = 1.05, \pval{} = 0.617).
...
A productivity-conditioned lag model directly tests the reverse-causal account: adding the lagged annual production trend to the lag-2 Cox model produces only 6.9\% attenuation of the stability effect (from \HR{} = 1.23 to \HR{} = 1.21, \pval{} = 0.006), with the conditional HR in year 10 remaining significant at 1.20 (\pval{} = 0.008).
...
The lagged stability $\times$ career stage consistently outperforms the lagged productivity in the prediction outside the sample ($\Delta$AUC = $+0.07$ to $+0.42$ for lags 1--5).
A nested model sequence confirms that the post-decade stability effect is not an artifact of omitted variables (stability \HR{} = 1.35--1.39 in five progressively controlled specifications, all \pval{} $\leq$ 0.003; Appendix Table~\ref{tab:nested_models}).
...
The pooled stability coefficient of 1.18 averages over a null effect early (\HR{} = 1.04) and a substantial effect later (\HR{} = 1.39 in year 20), describing a statistical artifact that does not correspond to no actual career stage.
...
The propensity-matched closure analysis produces a matched OR of 0.41 (\pval{} $< 0.001$) with a dose-response gradient (OR $= 0.77$, \pval{} = 0.039) that survives productivity controls.
These results support Hypothesis~2b, although definitive causal decomposition requires future designs with more exogenous variation.
Hazard ratios are best read as upper bounds in any forward-acting effect.
...
A period-adjusted plateau definition (conditional stability \HR{} at year 10 = 1.18, \pval{} = 0.007) and a regime-interaction model (Wild-Wald-Joint \pval{} = 0.495, with directionally positive stability effects in three of four regimes) confirm that the pattern is not period-specific.
...
The gradient continues beyond P90: at the extreme of the achievement distribution (approximately CV $\approx$ 135), the conditional stability \HR{} peaks at 1.80 (\pval{} = 0.049), although the point estimates in this range carry wide confidence intervals due to the lack of data.
...
The complete results, including logic-specific and archetype-specific estimates, are reported in the Appendix~\ref{app:institutional_logic}.
...
The HMM identifies a stability OR of 0.54 (\pval{} = 0.032) for remaining in the Growth state; XGBoost identifies the feature-importance crossover between network size and stability in career year 11.4, closely matching the Cox model's year-10 threshold (mean AUC = 0.873).
...
The film industry satisfies the same structural conditions as the primary setting: high centralization of the gatekeeping (directors select actors for roles, analogous to curators selecting artists; Gini = 0.745), the dependency of cumulative endorsement (credit-based career advancement) and observable institutional encounters (director-level casting records).
...
For each candidate $\tau \in \{5, 6, \ldots, 20\}$, the phase-specific stability coefficients were estimated using separate Cox models, and the optimal $\tau^*$ was selected by AIC minimization.
...
In Korean art data, conditional stability \HR{} increases monotonically from 1.068 (\pval{} = 0.468) at career onset to 1.188 (\pval{} = 0.008) in year 10 and 1.478 (\pval{} = 0.024) in year 20 -- nearly matching the linear interaction result.
In IMDb data, the conditional \HR{} is directionally consistent (1.058--1.072 throughout career years) but does not reach significance, reflecting the same limitation of structural power as the linear interaction.
The change-point model is therefore the preferred unified specification, as it allows a discrete structural break rather than imposing a smooth moderation (Figure~\ref{fig:unified_spec}).

\paragraph{Phase-split analysis.}
At the data-driven cut-off point, phase-split analysis reveals the predicted directional reversal: in the pre-decade phase, stability is marginally protective (\HR{} = 0.759, \pval{} = 0.082), while in the post-decade phase, stability \textit{significantly increases} plateau hazard (\HR{} = 1.150, \pval{} = 0.019).
...
A likelihood ratio test that compares the pooled model with fully phase-specific models confirms structural breakdown (LR $\chi^2$ = 16.34, $df$ = 4, \pval{} = 0.003; $df$ = 4 because all four covariates are estimated separately in each phase).

\paragraph{Why the linear interaction is null: empirical demonstration.}
For completeness, the continuous \textit{linear} interaction is also reported: stability $\times$ career year is not significant (\HR{} = 1.015, \pval{} = 0.800), and the interaction model does not improve the fit (LR $\chi^2$ = 0.19, \pval{} = 0.980).
Two complementary analyzes demonstrate that this null result reflects structural limitations of linear parameterization in the IMDb career distribution, not the absence of temporal variation (Table~\ref{tab:power_restriction}).

First, \textit{analytical power analysis}.
The Korean interaction coefficient ($\beta = 0.113$) was transferred to the standard error structure of IMDb (SE = 0.058).
At $\alpha = 0.05$, the linear interaction test has only 49.8\% power to detect the size of the Korean effect in the IMDb career distribution, much below the conventional 80\% threshold.
The minimum detectable effect for 80\% power would require an interaction coefficient 1.4$\times$ of the Korean estimate (\HR{} $\geq$ 1.176).
...
This shows that the finding is not dependent on the focal 10-year threshold.

See Appendix~\ref{app:imdb_pilot} for data construction and reproducibility.

\input{data/imdb_replication_table}

\input{data/imdb_cutpoint_robustness}

\input{data/unified_specification_table}
...
\input{data/power_and_restriction_table}
...
The pooled stability coefficient of 1.18 (\pval{} = 0.005) averages over a null effect in the first decade (\HR{} = 1.04) and a substantial effect thereafter (\HR{} = 1.39 in year 20) -- a range in which the pooled estimate falls at no actually observed career stage.
...
The theory generates testable predictions for any field that satisfies three structural conditions: gatekeeping centralization, cumulative endorsement dependence, and observable institutional encounters.
...
Three independent analyzes address the concern that different specifications are selected to match different datasets.
...
Second, the linear interaction is null in IMDb (\HR{} = 1.015, \pval{} = 0.800), but analytical power analysis shows that the linear test has only 49.8\% power to detect the size of the Korean effect ($\beta = 0.113$) given the standard error structure of IMDb (SE = 0.058).
The minimum detectable effect for the 80\% power is 1.4$\times$ the Korean estimate.

When IMDb actors are restricted to careers $\leq$ 15 years (pre-decade = 86.8\%), the continuous linear interaction becomes significant (\HR{} = 2.050, \pval{} = 0.012), and significance persists at all restriction levels ($\leq$ 20 years: \HR{} = 1.887, \pval{} = 0.035; $\leq$ 30 years: \HR{} = 1.642, \pval{} = 0.002).
...
At the data-driven cut-off point, the IMDb data reveal the predicted directional reversal: stability is marginally protective early (\HR{} = 0.752, \pval{} = 0.057) but significantly increases plateau risk thereafter (\HR{} = 1.145, \pval{} = 0.019).
Combined with a high director concentration (Gini = 0.745), these results provide evidence consistent with theory in a second field with comparable gatekeeping centralization.
...
The theory would be weakened by: (a) no phase reversal in fields with equivalent gatekeeping centralization (e.g., performing arts, academic medicine); (b) a phase reversal in decentralized fields where gatekeeping is minimal; or (c) onset timing that does not coincide with gatekeeping centralization across fields.
...
The analyzes in Section~\ref{sec:causal} provide convergent evidence for a forward-acting component, but definitive causal decomposition remains a task for future designs with larger pools of exogenous variation.
...
Second, the ``'' designated sample `` master restricts inference to within-elite stratification, a feature for internal validity but a limitation for external validity.
...
Fourth, empirical evidence comes primarily from a single field (Korean art), although Section~\ref{sec:pilot_replication} provides a pilot replication in the film industry (IMDb) where phase reversal replicates with a statistically significant structural break (\pval{} = 0.014).
...
The pooled stability coefficient of 1.18 averages over a null effect in the first decade (\HR{} = 1.04) and a substantial effect in the mid-to-late career (\HR{} = 1.39 in year 20), describing a statistical artifact that does not correspond to the actual career stage.
...
A data-driven change-point model applied identically to both datasets identifies overlapping career-year thresholds (Korean: $\tau^* = 10$ years, IMDb: $\tau^* = 7$ years; bootstrap CI overlap) without researcher-specified cutpoints.
In phase-split analysis at these thresholds, the stability of the director-network is marginally protective at the beginning (\HR{} = 0.752, \pval{} = 0.057) but significantly increases the plateau hazard afterward (\HR{} = 1.145, \pval{} = 0.019), and a formal structural break test rejects the null of equal coefficients (\pval{} = 0.009).
...
The complete reproducibility package---including all statistical analysis code (Python), the enriched bilingual dataset (JSON), the web-crawling scripts, and the data-construction pipeline---is publicly available at \url{https://github.com/joonhyungbae/TITV}. 
...
Each field is stored in bilingual form (\texttt{substrates \_ko} and \texttt{\_en} , preserving the original Korean text along with the English translation for verification.
...
All institution names are assigned official English equivalents through curated translation dictionaries, yielding a fully bilingual dataset.
...
The pilot replication uses IMDb Non-Commercial Datasets (\url{https://datasets.imdbws.com/}), a publicly accessible film industry database.
...
The plateau is defined as a five-year gap without film credits.
The career year is computed as the credit year minus the first credit year.
...
An XGBoost classifier (12 covariates, grouped 5-fold CV; mean AUC = 0.873) identifies the crossover of importance of the feature between network size and network stability in career year 11.4 -- closely matching the Cox model's year-10 threshold.
